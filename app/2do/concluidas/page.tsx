...
import { ArrowLeft, CheckCircle, ListTodo, Trash2, Undo2, X } from 'lucide-react';
...
export default function Do2ConcluidasPage() {
        ...
        <main className="flex-1 max-w-4xl w-full mx-auto p-4 md:p-8 flex flex-col">
          <div className="mb-8">
            <h1 className="text-2xl font-bold text-white flex items-center gap-2">
              <CheckCircle className="text-emerald-500" /> Tarefas Concluídas
            </h1>
            <p className="text-sm text-slate-400 mt-1">Histórico das suas atividades finalizadas.</p>
          </div>
            ...
            <div className="p-2 flex-1 overflow-y-auto">
              {loading ? (
                ...
                <div className="p-8 text-center text-slate-500 flex flex-col items-center">
                  <CheckCircle size={32} className="mb-3 opacity-20 text-emerald-500" />
                  <p>Nenhuma tarefa concluída.</p>
                </div>
              ) : (
                <ul className="space-y-2">
                  {tarefas.map(tarefa => (
                    <li key={tarefa.id} className="flex items-center gap-3 p-3 rounded-lg border bg-slate-900/50 border-slate-800/50 opacity-80 transition-all hover:opacity-100">
                      
                      <CheckCircle size={20} className="text-emerald-500 shrink-0" />

                      <span className="flex-1 text-sm line-through text-slate-500">
                        {tarefa.descricao}
                      </span>
                      ...
                      <div className="flex items-center gap-2 shrink-0">
                        <button 
                          ...
                          title="Voltar para tarefas pendentes"
                        >
